...
- **Vitalparameter over time**: SpO₂, heart rate, non-invasive blood pressure (NiBP) and
  temperature, all on one shared timeline so trends and correlations with events stay visible.
- **Medications and fluids**: *Bolus* (single dose) and *Dauerinfusion* (continuous infusion),
  each timestamped.
- **Phases and events**: the milestones of a case — *Narkosebeginn* (anesthesia start), *Schnitt*
  (incision), *Naht* (suturing), *Ausleitungsende* (end of emergence), *Entlassung* (discharge).

The interface is German throughout, and this document keeps the clinical terms in German so it
reads against the same vocabulary the screen uses.
...
- **Recording** starts on the row being written into, and the row's own name is what starts it:
  ...
  open that row's entry sheet. Values are typed on a keypad — see
  [Entering a value](#entering-a-value).
- **Vitalparameter** are drawn as one lane each over a shared time axis, medications and phase
  events in bands beneath, and the newest reading on each lane is set large in a rail to the right
  — see [Timeline](#timeline).
- **Corrections** are made by dragging a point, by arrow keys once selected, or by reopening the
  entry in the sheet it was written in — including ending a running *Dauerinfusion*. The audit
  trail is kept on every entry and shown in that sheet, in front of whoever is about to correct it
  again. On a touchscreen a swipe scrolls the record and a press and hold is what moves a value.
...
| `bolus` | a single dose (*Bolus*) | point in time |
| `infusion` | a continuous rate (*Dauerinfusion*) | interval, open while running |
| `event` | a phase milestone (*Ereignis*) | point in time |

Blood pressure is three `vital` entries — systolic, *MAD* (*Mittlerer arterieller Druck*, the
...
- **Entry flow**: what is being written down is chosen by pointing at its row, not by naming it in
  a list, and the entry timestamps to now by default — documentation in the OR is logged as it
  happens rather than placed at an arbitrary spot on the chart. Tapping an existing point reopens
  it in the same control. Detail in [Entering a value](#entering-a-value).
- **NiBP (blood pressure)** is entered as one reading holding all three pressures on one shared
  ...
  off for a manual cuff, which gives no MAD. They are still *stored* as three ordinary vital
...
read in rather than everything a patient could produce, so an ordinary case fills its lane instead
of drawing a flat line across an empty box. They do not rescale to the data, because two cases have
to be comparable at a glance. A reading outside its lane is drawn hollow against the edge it went
past, keeping its own number in the readout and in the written-out values — nothing may vanish from
a clinical record because of a drawing choice.

Phase events are drawn as dashed rules through every lane, so the vitals at *Schnitt* can be read
...
down. The pressure lane shows the pair with the MAD beneath it, and where a part of a reading has
...
pressures are told apart by marker shape (systolic points up, diastolic down, the MAD is a dot) as
...
which is what keeps the tallest form in the app — a *Dauerinfusion*, with a rate, a unit, a
...
— `120/70 (85)`. Each pressure has a `gemessen` checkbox, because a manual cuff gives no MAD, and
...
the MAD — which has no pre-operative counterpart — opens switched off rather than on a number the
app made up.
...
| `record-a-bolus` | A *Bolus* with its drug, amount and unit, appearing in the medication band |
| `run-an-infusion-stop-it-and-undo` | A *Dauerinfusion* documented while it runs, ended from the band, and the ending taken back |
